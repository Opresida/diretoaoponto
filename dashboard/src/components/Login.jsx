...
const ALLOWED = ["coordinator", "manager", "admin", "statistician"];
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!ALLOWED.includes(r.user.role)) {
        setError("Acesso restrito a coordenação/estatística.");
        return;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-sm">
        ...
        <p className="text-xs text-slate-400 text-center mb-5">Apuração em Tempo Real · Coordenação</p>
