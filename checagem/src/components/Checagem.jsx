...
export default function Checagem({ user, onLogout }) {
  ...
  const load = async () => {
    setLoading(true);
    try { setQueue((await api.queue()).queue ?? []); } catch { /* ignore */ } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const open = async (item) => {
    setSel(item); setMedia(null); setReason(""); setMethod("audio"); setMsg("");
    try { setMedia(await api.media(item.interview_id)); }
  ...
  const decide = async (result) => {
    ...
    setBusy(true); setMsg("");
    try {
      await api.result(sel.check_id, { result, reason: reason.trim() || undefined, method });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <header className="flex items-center justify-between gap-2 p-3 sm:p-4 border-b border-slate-800 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={load} className="btn-secondary px-2.5 py-2" title="Atualizar"><RefreshCw size={15} className={loading ? "animate-spin" : ""} /></button>
          <button onClick={onLogout} className="text-slate-400 p-2"><LogOut size={18} /></button>
        </div>
      </header>

      <div className="max-w-7xl mx-auto p-3 sm:p-4 grid gap-4 lg:grid-cols-[380px_1fr]">
        {/* FILA */}
        <div className="card p-3 min-w-0">
          <div className="font-semibold text-sm mb-2 px-1">Fila priorizada</div>
